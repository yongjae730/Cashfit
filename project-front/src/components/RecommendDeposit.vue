<template>
  <v-card style="margin: 10px" class="my-6 info-card">
    <v-card-title class="d-flex align-center font-weight-bold text-h6">
      맞춤 예금 상품
      <v-spacer></v-spacer>
      <v-chip color="primary" class="ml-2">{{ userInfo.age }}세 맞춤</v-chip>
    </v-card-title>
    ...
    <v-card-text>
            ...
            <v-card width="400" class="ma-2 product-recommendation-card">
              <v-card-title class="bank-info d-flex align-center">
                <div class="bank-logo-container mr-3">
                  <v-img :src="`/bank_images/${product.kor_co_nm}.jpg`" :error-src="`/bank_images/default.jpg`" class="bank-logo" cover alt="상품 이미지"></v-img>
                </div>
      ...
      <div v-else class="empty-state">현재 추천 가능한 상품이 없습니다.</div>
    </v-card-text>
  </v-card>
</template>
...
const fetchRecommendations = async () => {
  try {
    ...
    const response = await axios.get(`${API_URL}/api/financials/recommendations/age/`, { headers });
    console.log("추천 상품 조회 결과:", response.data);

    // 적금 데이터만 저장
    recommendations.value = response.data.recommendations.deposits || [];
  } catch (error) {
...
const isProductLiked = (productId) => {
  return accountStore.user?.liked_products?.some((product) => product.id === productId) || false;
};
...
const toggleLike = async (productId) => {
  try {
    ...
    if (isProductLiked(productId)) {
      ...
      // 좋아요 상태가 아니면 추가
      await axios.post(`${API_URL}/api/financials/products/${productId}/like/`, {}, { headers });
    }
